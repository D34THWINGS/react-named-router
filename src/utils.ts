...
export type NamedRouteConfig = CustomNamedRouteConfig &
...
export const mapRoutes = (
  ...
  routes.forEach(route => {
    ...
    (Array.isArray(route.name) ? route.name : [route.name]).forEach(name => {
      let regex: { regex: RegExp } | undefined;
      if (route.path) {
        regex = {
          regex: new RegExp(
            `^${(basename + route.path)
...
export class RoutingContext {
  ...
  public constructor(routesMap: Map<string, ExtendedRouteConfig>, routerContext: Partial<RouteChildrenProps> = {}) {
    ...
    this.exactParams = this.currentRoute
      ...
      : defaultParams;
    const matchAllRoute = routerContext.location ? this.match(routerContext.location.pathname, true) : null;
    this.params = this.currentRoute
      ? RoutingContext.extractParams(matchAllRoute, this.location.pathname)
      : defaultParams;
  }

  public match = (pathname: string, matchAll?: boolean) =>
    Array.from(this.routesMap.values()).find(
      route => (matchAll || (!matchAll && route.exact)) && !!route.regex && route.regex.test(pathname),
    ) || null;
  ...
  public push = (location: string | NamedLocation, params?: { [key: string]: string }) =>
    this.history.push({
  ...
  public replace = (location: string | NamedLocation, params?: { [key: string]: string }) =>
